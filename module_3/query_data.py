...
def pct(x):
    return f"{x:.2f}%"  # Convert to present
...
def main():
    ...
    q2 = sql_query("SELECT ROUND(100.0 * COUNT(*) FILTER (WHERE us_or_international ILIKE %s) / COUNT(*), 2) AS pct_international FROM applicants WHERE us_or_international " \
    "NOT ILIKE %s AND us_or_international NOT ILIKE %s;", "%internat%", "%american%", "%other%")
...
"""
    with open("query_summary.md", "w", encoding="utf-8") as f:
        f.write(summary)
    print("Wrote query_summary.md")
